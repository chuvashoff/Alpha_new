...
def create_reports_v80(sl_object_all: dict, node_param_rus: str, node_alg_name: str, sl_param: dict):
    ...
    for obj in sl_object_all:
        ...
        if not os.path.exists(os.path.join('File_for_Import', 'Reports_v80')):
            os.mkdir(os.path.join('File_for_Import', 'Reports_v80'))
        # Сохраняем
        wb.save(os.path.join('File_for_Import', 'Reports_v80', f'{node_param_rus} {obj[1]}.xlsx'))

    return


def create_reports_pz_v80(sl_object_all: dict, node_param_rus: str, node_alg_name: str, sl_param: dict):
    # sl_object_all =
    # { (Объект, рус имя объекта, индекс объекта): {контроллер: (ip основной, ip резервный, индекс объекта)} }
            ...
            if cpu in sl_object_all[obj]:
                ...
                for par, property_par in sl_par.items():
                    ...
                    for i, vert_alig in {'P': 'center', 'Q': 'left', 'R': 'center', 'S': 'center',
                        ...
                        sheet[f'{i}{num_cel}'].alignment = Alignment(horizontal=vert_alig, vertical="center",
                                                                     wrap_text=True)
                    sheet.row_dimensions[num_cel].height = 21
                    num_cel += 1
                    num_par += 1
                    igrek_delta -= 1
                    sheet.row_dimensions[num_cel].height = 20  # !!! увеличиваем т
        # Прячем столбцы с привязками
...
def create_reports_sday_v80(sl_object_all: dict, node_param_rus: str):
    igrek = 33
    igrek_delta = 24
    sl_param = dict()
    ...
    for obj, sl_par in sl_param.items():
        ...
        # Сохраняем
        wb.save(
            os.path.join('File_for_Import', 'Reports_v80', f'{node_param_rus} {sl_name_object.get(obj, str(777))}.xlsx')
        )

    return


def create_reports_sut_v80(sl_object_all: dict, node_param_rus: str):
    igrek = 33
    igrek_delta = 24
    sl_param = dict()
    # os.path.abspath(os.curdir) os.path.dirname(sys.argv[0])
    if os.path.exists(os.path.join(os.path.abspath(os.curdir), 'SCADA', 'Daily')):
        all_obj = tuple([i[0] for i in sl_object_all])
        for file in os.listdir(os.path.join(os.path.abspath(os.curdir), 'SCADA', 'Daily')):
            if file[:file.find('_')] in all_obj and file.endswith('DailyList.csv'):
